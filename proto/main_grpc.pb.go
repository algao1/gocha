...
func (c *chatClient) Stream(ctx context.Context, opts ...grpc.CallOption) (Chat_StreamClient, error) {
	stream, err := c.cc.NewStream(ctx, &Chat_ServiceDesc.Streams[0], "/proto.Chat/Stream", opts...)
	if err != nil {
		return nil, err
	}
...
func (x *chatStreamClient) Send(m *Note) error {
	return x.ClientStream.SendMsg(m)
}

func (x *chatStreamClient) Recv() (*Note, error) {
	m := new(Note)
...
func (x *chatStreamServer) Send(m *Note) error {
	return x.ServerStream.SendMsg(m)
}

func (x *chatStreamServer) Recv() (*Note, error) {
	m := new(Note)
